    ...
    return <div className="py-5 desktop:py-20  flex flex-col rounded-md bg-slate-50 w-full  w-full desktop:max-w-[20%] relative">
        ...
        <div className='px-5 my-2'>
                {issues?.map((element,index)=>{
                 ...
                 return <Link key={index} href={`/publications?issue=${element?.issue}`} className='flex items-center p-2 rounded-md hover:bg-slate-200  justify-between duration-500 text-sm desktop:text-base '>
                       {`Issue ${element?.issue}`} 

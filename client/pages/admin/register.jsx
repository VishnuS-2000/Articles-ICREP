...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!account.email || !account.firstName || !account?.lastName || !role) {
      ...
      if (!role) setErrorFields((prev) => { prev[0] = true; return prev })
      if (!account.firstName) setErrorFields((prev) => { prev[1] = true; return prev })

      if (!account.lastName) setErrorFields((prev) => { prev[2] = true; return prev })

      if (!account.email) setErrorFields((prev) => { prev[3] = true; return prev })
    ...
    <div className="min-h-screen tablet:bg-gradient-to-r from-primary to-blue-800 flex flex-col w-full justify-center  tablet:items-center">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {!success ? <div className="flex flex-col w-full bg-white p-5  space-y-5 tablet:px-10 tablet:py-20 tablet:drop-shadow-lg  tablet:rounded-md tablet:w-[560px]   desktop:space-y-6">
          ...
          <FormControl isInvalid={errorFields[0]} isRequired={true}>
          ...
          <FormControl className='flex space-x-3' >
            <FormControl isInvalid={errorFields[1]} >
              ...
              <Input type="text" variant="filled" value={account?.firstName} onChange={(e) => { setAccount({ ...account, firstName: e.target.value }); if (e.target.value && errorFields[1]) { setErrorFields((prev) => { prev[1] == false; return prev }) } }} />
              {errorFields[1] && <FormErrorMessage>First Name required</FormErrorMessage>}
            </FormControl>

            <FormControl isInvalid={errorFields[2]}>
              <FormLabel >Last Name</FormLabel>
              <Input type="text" variant="filled" value={account?.lastName} onChange={(e) => { setAccount({ ...account, lastName: e.target.value }); if (e.target.value && errorFields[1]) { setErrorFields((prev) => { prev[2] == false; return prev }) } }} />
              {errorFields[2] && <FormErrorMessage>Last Name required</FormErrorMessage>}
          ...
          <div className="flex flex-col w-full justify-center  bg-white p-5  space-y-12 tablet:px-10 tablet:py-20 tablet:drop-shadow-lg  tablet:rounded-md tablet:w-[560px]   desktop:space-y-8">
            ...
            <p className='text-xl italic '>Request for new account submitted.Status will initimated through email</p>
